...
def sdf_diff_loss(pred, label, weight, scale, l2_loss=True):
    ...
    return loss
def depth_bce_loss(label,pred):
    config = SHINEConfig()

    #losses=(1-F.cosine_similarity(label,pred)).sum()
    losses = (torch.abs(1 - F.cosine_similarity(label, pred))).sum()

    '''
    loss = torch.cross(label,pred,dim=1)
    pred_length = torch.norm(pred, dim=1)
    loss_a=torch.norm(loss,dim=1)
    losses=(loss_a/pred_length)
    mask=~(torch.isnan(losses))
    losses=losses[mask]
    if losses.numel()==0:
        losses=torch.tensor(0,device=config.device)
    else:
        losses=losses.sum()
    '''
...
def sdf_bce_loss(pred, label, sigma, weight, weighted=False, bce_reduction = "mean"):
    ...
    return loss
def ocup_bce_loss(pred, label, sigma, weight, weighted=False, bce_reduction = "mean"):
    config=SHINEConfig()
    if weighted:
        loss_bce = nn.BCEWithLogitsLoss(reduction=bce_reduction, weight=weight)
        #loss_bce = nn.BCELoss(reduction=bce_reduction, weight=weight)
    else:
        loss_bce = nn.BCEWithLogitsLoss(reduction=bce_reduction)
        #loss_bce = nn.BCELoss(reduction=bce_reduction, weight=weight)
    k=0.1
    label_o=label / config.scale
    #label_o=torch.nn.Tanh(label_o)
    label_o=(torch.exp(-label_o**2/(2* k**2))-0.5)*2
    label_op=torch.sigmoid(label_o)

    #pred=pred
    #pred=(torch.exp(-pred_1**2/(2* k**2))-0.5)*10
    pred=torch.sigmoid(pred)
    #loss=(torch.abs(label_op-pred)).sum()
