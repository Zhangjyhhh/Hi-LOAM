...
class SHINEConfig:
    def __init__(self,clear_table):

        # Default values

        #  tracking settings
        self.num_iteration: int = 100  # for tracking iteration
        self.tracking_learning_rate: float = 0.018
        self.tracking_bs: int = 16384 #
        self.lm_lambda: float = 1e-3 #0.0001, pinslam is 1e-4
        self.reg_term_thre_deg: float = 0.001 #0.01
        self.reg_term_thre_m: float = 0.0005 #0.0005
        self.pool_capacity: int = int(2e7)  # like pin slam 2e7
        self.bs_new_sample: int = 2000
        #self.clear_table: int = 1000    #this should keep the same with mesh_freq_frame
        self.clear_table: int = clear_table    #this should keep the same with mesh_freq_frame
        self.intrinsic_correct: bool = False
        ...
        self.pc_count_gpu_limit: int = 50  # maximum used frame number to be stored in the gpu
        ...
        self.tracking_vox_down: float=0.2
        self.mapping_vox_down: float=0.1
        ...
        # semantic related
        self.sem_learning_rate: float = 0.05
        self.semantic_on: bool = False  # semantic shine mapping on [semantic]
        self.sem_class_count: int = 21  # semantic class count: 21 for semantic kitti
        ...
        self.feature_std: float =  0.0#0.05  # grid feature initialization standard deviation #
        self.sem_feature_std: float = 0.5384  # 随便设的
        self.poly_int_on: bool = True  # use polynomial interpolation or linear interpolation
        self.octree_from_surface_samples: bool = False  # Use all the surface samples or just the exact measurements to build the octree. If True may lead to larger memory, but is more robust while the reconstruction.
        ...
        self.clearance_dist_m: float = 0.3 #0.3
    ...
    def load(self, config_file):
        ...
        self.intrinsic_correct=config_args["setting"]["intrinsic_correct"]
        self.init_trans = config_args["setting"]["init_trans"]
        self.deskew = config_args["setting"]["deskew"]
        # process
        ...
        self.tracking_vox_down = config_args["process"]["tracking_vox_down"]
        self.mapping_vox_down = config_args["process"]["mapping_vox_down"]
        self.pool_capacity = int(float(config_args["process"]["pool_capacity"]))
        self.bs_new_sample = config_args["process"]["bs_new_sample"]
        ...
        self.sem_feature_dim = 32  # 后期可以加在配置文间里面
        ...
        self.normal_loss_on = config_args["loss"]["normal_loss_on"]
        ...
        self.iters = config_args["optimizer"][
            "iters"]  # maximum iters (in our implementation, iters means iteration actually)
        ...
        self.o3d_vis_on = config_args["eval"][
            "o3d_vis_on"]  # turn on the open3d visualizer to visualize the mapping progress or not
        self.vis_freq_iters = config_args["eval"][
            "vis_freq_iters"]  # frequency for mesh reconstruction for batch mode (per x iters)
        self.save_freq_iters = config_args["eval"][
            "save_freq_iters"]  # frequency for model saving for batch mode (per x iters)
        self.mesh_freq_frame = config_args["eval"][
            "mesh_freq_frame"]  # frequency for mesh reconstruction for incremental mode (per x frame)
        self.mc_with_octree = config_args["eval"][
            "mc_with_octree"]  # using octree to narrow down the region that needs the sdf query so as to boost the efficieny
